...
class Austin(Dataset):
    """
    CAISO dataset. Instantaneous power reported in MW.
    ...
    def __init__(self,path = '.',start_idx = 0, end_idx = 9999999,
        ...
        if 'austin_dict.pkl.pgz' not in os.listdir(path):
        ...
        with pgzip.open(os.path.join(path,'austin_dict.pkl.pgz')) as f:
            sd = pickle.load(f)
