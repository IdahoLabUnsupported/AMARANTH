...
#
# Power disaggregation helper functions
###############################################################################
import numpy as np
...
<<<<<<< HEAD
def kl_divergence_from_seqs(true_pred_df_original: pd.DataFrame, tgt_fields: list, train_cutoff, pred_fld_suffix: str = '_pred') -> pd.DataFrame:
=======
...
<<<<<<< HEAD
    true_pred_df = true_pred_df_original.copy()
=======
>>>>>>> main
...
<<<<<<< HEAD
def simulate_model_weight_attack(tgt_model, testing_data: pd.DataFrame, wgt_grads: list, perturb_schedule_lambda: float = 1.0, window_size: int = 1440, 
                                 n_targets: int = 4, magnitude_scale_min: float = 0.005, magnitude_scale_max: float = 0.01) -> pd.DataFrame:

    assert magnitude_scale_max > magnitude_scale_min, "`magnitude_scale_max` must be strictly greater than `magnitude_scale_min`"
=======
...
<<<<<<< HEAD
                    wgt_magnitude = np.random.uniform(magnitude_scale_min, magnitude_scale_max)
=======
